...
class MoveToRedObject(Waitable):
    ...
    def move_to_red(self):
        ...
        #distance = msg.data
        rospy.loginfo("distance from object : {}".format(distance))
        distance = max(0, distance-self.MAX_DISTANCE_FROM_OBJECT-self.ARM_LENGTH)
        ...
        self.moving=True
        MovementManager(distance,False).wait_for_cb()
