...
class ObjectDetector(Waitable):
    ...
    def get_red_image_cords(self, cords):
        ...
        """
	while self.image_depth is None:
		time.sleep(0.1)
        self.cords = cords
	if cords.radius == 0:
        ...
        self.callback_called = True
        rospy.loginfo("Distance from red object : {}".format(self.distance))
	...
	if self.cords.radius == 0:
	    self.angle=False
	    return
	rospy.logdebug("{},{}".format(image_data.width, image_data.height))
	...
	rospy.loginfo("orig angle : {}".format(angle))
	"""
	if angle < 0:
	    angle = 360+angle
	    angle *= 1.1
    ...
    def handle_image_color(self, image_data):
        ...
        """
        rate = rospy.Rate(1)
        rospy.loginfo("handle_image_color")
    ...
    def handle_image_depth(self, image_data):
        ...
        """
        rospy.loginfo("handle_image_depth")
        cv_bridge = CvBridge()
